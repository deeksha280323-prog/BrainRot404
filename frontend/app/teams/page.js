    ...
    <div className="max-w-7xl mx-auto px-6 py-12 relative h-full">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pb-12">
          {teams.map((team, i) => (
            <motion.div
              ...
              className="bg-white border border-[#f0e6e4]/60 rounded-3xl flex flex-col group overflow-hidden shadow-xl shadow-[#f9ae9b]/5 hover:border-[#f9ae9b]/40 transition-all"
            >
              <div className="h-1.5 w-full bg-gradient-secondary opacity-30 group-hover:opacity-100 transition-opacity"></div>
              
              <div className="p-8 flex-grow flex flex-col">
                <div className="flex items-start justify-between mb-6">
                  <h3 className={`${playfair.className} text-2xl font-bold text-[#222] truncate pr-4`}>{team.name}</h3>
                  <div className="flex items-center gap-2 bg-[#fbfbfb] border border-[#eee] px-3 py-1.5 rounded-xl text-[10px] font-black text-gray-400 shrink-0 shadow-sm">
                    <span className="text-[#f9ae9b]">👤 {team.members?.length || 1}</span>
                ...
                {team.hackathonTrack && (
                  <div className="mb-6">
                    <span className="text-[10px] font-bold px-3 py-1.5 rounded-xl bg-[#fff2ee] text-[#f9ae9b] border border-[#f9ae9b]/10 uppercase tracking-widest">
                      🎯 {team.hackathonTrack}
                    </span>
                  </div>
                )}
              ...
              <div className="p-6 bg-[#fbfbfb]/50 border-t border-[#f0e6e4]/40 flex gap-4">
                <Link 
                  href={`/chat/${team._id}`} 
                  className="flex-1 text-center py-3.5 bg-white hover:bg-gray-50 border border-[#f0e6e4] text-[11px] font-bold uppercase tracking-widest text-gray-400 rounded-2xl transition-all shadow-sm active:scale-95"
                >
                  Briefing
                </Link>
                <button
                  ...
                  className="flex-1 py-3.5 bg-[#f9ae9b] hover:bg-[#fac2b4] text-white text-[11px] font-bold uppercase tracking-widest rounded-2xl transition-all shadow-lg shadow-[#f9ae9b]/10 disabled:opacity-50 active:scale-95"
                >
                  {joiningTeam === team._id ? "..." : "Request Sync"}
                </button>
              </div>
            </motion.div>
          ))}
        ...
        <div className="text-center py-24 bg-white border border-dashed border-[#f0e6e4] rounded-[2.5rem] shadow-inner">
          ...
          <p className="text-gray-400 font-bold uppercase tracking-widest text-xs">The hangar is currently empty.</p>
          <p className="text-sm text-gray-300 mt-2">Be the first to launch a technical squad and invite your counterparts!</p>
        </div>
      )}
